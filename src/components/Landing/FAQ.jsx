...
function FAQ({ language }) {
  ...
  return (
    <section className="w-full flex flex-col items-center justify-center py-16 md:py-24 bg-transparent px-8">
      {/* FAQ Heading */}
      <h2 className="text-3xl md:text-48 font-extrabold text-gray-900 dark:text-white text-center mb-3 tracking-tight">{heading}</h2>
      <p className="text-gray-500 dark:text-gray-300 text-center mb-10 max-w-xl text-16">
        {subheading}
      </p>
      {/* FAQ Accordions */}
      <div className="w-full max-w-2xl flex flex-col gap-4 mb-20">
        {faqs.map((item, idx) => (
          <div key={idx} className="rounded-lg bg-gray-50 dark:bg-neutral-900 overflow-hidden">
            ...
            >
              <span className="text-18 font-semibold">{item.q}</span>
              {open === idx ? <FaChevronUp className="ml-2" /> : <FaChevronDown className="ml-2" />}
            </button>
            <div
              className={`px-6 pb-4 text-18 text-gray-600 dark:text-gray-300 text-base transition-all duration-300 ${open === idx ? 'block' : 'hidden'}`}
            >
              {item.a}
            </div>
          </div>
      ...
      <div className="container mx-auto">
        ...
        >
          <div className="flex-1">
            <h3 className="text-2xl md:text-48 font-extrabold text-white mb-3">{ctaHeading}</h3>
            <p className="text-gray-300 text-base md:text-16 mb-2 md:mb-0">{ctaText}</p>
          </div>
          <div className="flex-shrink-0 w-full md:w-auto flex justify-center md:justify-end">
            <CommonButton
              text={ctaButtonText}
              className="px-8 py-3 text-16 font-bold rounded-full"
              iconPosition="right"
            />
          </div>
        </div>
