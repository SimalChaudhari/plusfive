...
function WhatHappens({ language }) {
  ...
  return (
    <section className="w-full flex flex-col items-center justify-center py-16 px-8 md:px-0 bg-white dark:bg-customGray">
      {/* Heading */}
      <h2 className="text-3xl md:text-48 font-extrabold text-center text-gray-900 dark:text-white mb-4">
      ...
      {/* Subheading */}
      <p className="text-center text-gray-500 dark:text-gray-300 max-w-2xl mb-10 mx-auto text-16">
        {t.subheading}
      </p>
      {/* Table/Card */}
      <div className="w-full overflow-x-auto mx-auto my-8">
        <div className="min-w-[700px] max-w-4xl bg-white dark:bg-customBlack rounded-2xl shadow-lg border border-gray-200 dark:border-customBorderColor mx-auto p-6 md:p-8">
          ...
          <div className="grid grid-cols-2 gap-0 relative bg-transparent">
            <div className="font-bold text-20 dark:bg-customGray bg-gray-900 text-white px-6 py-4 w-full block rounded-l-2xl ">{language === 'he' ? 'מה תקבל' : 'What You Get'}</div>
            ...
            <div className="font-bold text-20 dark:bg-customGray bg-gray-900 text-white px-6 py-4 w-full block rounded-r-2xl ">{language === 'he' ? 'איך זה עובד' : 'How It Works'}</div>
          </div>
            ...
            {t.rows.map((row, i) => (
              <div key={i} className={`grid grid-cols-2 items-center border-t border-gray-100 dark:border-customBorderColor bg-white dark:bg-customBlack relative` + (i === t.rows.length - 1 ? ' rounded-b-2xl' : '')}>
                <div className="flex items-center gap-4 px-6 py-5">
                  ...
                  <span className="text-gray-900 dark:text-white text-base md:text-18 font-medium">{row.text}</span>
                </div>
                ...
                <div className="px-6 py-5 text-gray-700 dark:text-gray-300 text-base md:text-18">{row.how}</div>
              </div>
            ))}
          </div>
        </div>
