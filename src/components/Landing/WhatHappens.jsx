...
function WhatHappens() {
  return (
    <section className="w-full flex flex-col items-center justify-center py-16 px-2 md:px-0 bg-white">
      {/* Heading */}
      <h2 className="text-3xl md:text-5xl font-extrabold text-center text-gray-900 mb-4">
        Here's <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-orange-400">What Happens</span> When You<br className="hidden md:block" /> Plug In Plusfive
      </h2>
      {/* Subheading */}
      <p className="text-center text-gray-500 max-w-2xl mb-10 mx-auto">
      ...
      <div className="w-full overflow-x-auto mx-auto my-8">
        <div className="min-w-[700px] max-w-4xl bg-white rounded-2xl shadow-lg border border-gray-200 mx-auto p-6 md:p-8">
          {/* Header Row */}
          <div className="grid grid-cols-2 gap-0 relative bg-transparent">
            <div className="font-bold text-lg bg-gray-900 text-white px-6 py-4 w-full block rounded-l-2xl ">What You Get</div>
            ...
            <div className="font-bold text-lg bg-gray-900 text-white px-6 py-4 w-full block rounded-r-2xl ">How It Works</div>
          </div>
          {/* Divider below header */}
          {/* <div className="w-full border-b-2 border-gray-200"></div> */}
          <div>
            {rows.map((row, i) => (
              <div key={i} className={`grid grid-cols-2 items-center border-t border-gray-100 bg-white relative` + (i === rows.length - 1 ? ' rounded-b-2xl' : '')}>
                <div className="flex items-center gap-4 px-6 py-5">
                  <span className="bg-pink-50 rounded-lg p-2 flex items-center justify-center w-10 h-10">{row.icon}</span>
                  <span className="text-gray-900 text-base md:text-lg font-medium">{row.text}</span>
                </div>
                {/* Vertical divider for each row - only on md+ */}
                <div className="hidden md:block absolute top-0 bottom-0 left-1/2 w-0.5 bg-gray-200"></div>
                <div className="px-6 py-5 text-gray-700 text-base md:text-lg">{row.how}</div>
              </div>
            ))}
          </div>
        </div>
