...
function CustomDropdown({ value, options, onChange, className, label, width = 'w-20', showLabelInsteadOfValue }) {
  ...
  return (
    <div className={`relative ${width}`} ref={ref}>
      <button
        type="button"
        className={`flex items-center justify-between bg-gray-100 dark:bg-customBrown text-gray-900 dark:text-white rounded px-2 py-1 text-sm border border-gray-300 dark:border-gray-700 focus:outline-none ${className}`}
        onClick={() => setOpen(o => !o)}
      ...
      {open && (
        <ul className={`absolute z-40 mt-1 left-0 right-0 max-h-56 overflow-auto bg-white dark:bg-customBrown border border-gray-200 dark:border-gray-700 rounded shadow-lg ${width}`}
            role="listbox">
          {options.map((opt, idx) => (
            <li
              key={opt.value}
              className={`px-3 py-1.5 text-sm cursor-pointer hover:bg-pink-100 dark:hover:bg-pink-500/20 hover:text-pink-600 dark:hover:text-pink-400 ${opt.value === value ? 'bg-pink-100 text-pink-600 dark:bg-pink-500/20 dark:text-pink-400 font-semibold' : 'text-gray-900 dark:text-white'}`}
  ...
  return (
    <div className={`flex flex-col items-center min-w-[220px] ${className || ''}`}>
      ...
      <div className="grid grid-cols-7 gap-1 w-full text-xs mb-1">
        {['Su','Mo','Tu','We','Th','Fr','Sa'].map(d => <div key={d} className="text-center text-gray-400 dark:text-gray-500">{d}</div>)}
      </div>
      <div className="grid grid-cols-7 gap-1 w-full">
        ...
        {days.map(day => {
          ...
          return (
            <button
              key={day.toISOString()}
              className={`rounded-full w-8 h-8 text-xs text-center transition
                ${isSelected ? 'bg-pink-500 text-white font-bold' : inRange ? 'bg-pink-100 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}
                ${isSameDay(day, today) ? 'border border-pink-400 dark:border-pink-500' : ''}
...
function CommonDateRange({
  ...
  return (
    <div className="relative inline-block">
      <button
        type="button"
        className="flex items-center gap-2 px-4 py-2.5 bg-gray-50 dark:bg-customBrown border-2 border-gray-200 dark:border-customBorderColor rounded-xl text-gray-900 dark:text-white text-sm focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500"
          ...
          <div className="flex flex-1">
            ...
            <div className="flex flex-col w-48 border-r border-gray-200 dark:border-gray-700 pr-4">
              {SIDEBAR_RANGES.map((r, idx) => (
                <button
                  key={r.label}
                  className={`text-left px-3 py-2 rounded-lg text-sm mb-1 transition-colors dark:font-normal font-bold dark:text-white text-slate-500 ${activeQuick === idx ? 'bg-pink-100 text-pink-600 dark:bg-pink-500/20 dark:text-pink-400' : 'hover:bg-pink-50 dark:hover:bg-pink-500/10 hover:text-pink-600 dark:hover:text-pink-400'}`}
              ...
              <button
                className={`text-left px-3 py-2 rounded-lg text-sm mt-2 ${activeQuick === null ? 'bg-pink-200 text-pink-600 dark:bg-pink-500/20 dark:text-pink-400' : 'hover:bg-pink-50 dark:hover:bg-pink-500/10 hover:text-pink-600 dark:hover:text-pink-400'}`}
                disabled
              >
                Custom
              </button>
            ...
            <div className="flex flex-col flex-1 pl-4">
              <div className="flex gap-4 justify-center items-start">
                {/* Month navigation */}
                <button
                  className="h-8 w-8 flex items-center justify-center rounded-full border border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 mr-2 mt-8 bg-white dark:bg-customBrown"
                ...
                />
                <button
                  className="h-8 w-8 flex items-center justify-center rounded-full border border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 ml-2 mt-8 bg-white dark:bg-customBrown"
              ...
              <div className="flex justify-end gap-2 mt-4">
                <button
                  className="px-4 py-2 bg-gray-200 dark:bg-customBrown text-gray-700 dark:text-white rounded-lg text-sm hover:bg-gray-300 dark:hover:bg-pink-500/10"
